"""Focused diagnostic: fig11 τ=2/3 with turbo_iters and M variations.

Tests two SNR points (10, 12 dB) with K=3000, 15 frames.
"""
...
def main():
    ...
    configs = [
        ...
        ("M8_it30",     8,  3, 30, 20.0),
        ("M8_it50",     8,  3, 50, 20.0),
        ("M16_it20",   16,  3, 20, 20.0),
        ("M16_it30",   16,  3, 30, 20.0),
    ]

    print(f"Fig 11 diagnostic 2: tau=2/3, K={K}, frames={n_frames}")
    print(f"{'Config':<15} {'SNR':>6} {'BER':>10} {'Errors':>8} {'Bits':>8} {'Time':>8}")
    print("-" * 60)

    for eb_n0 in snr_points:
        for label, m_states, future_len, turbo_iters, llr_clip in configs:
            result = run_one_config(
                tau, eb_n0, K, n_frames,
                m_states, future_len, turbo_iters, llr_clip,
                isi_len, seed,
            )
            print(f"{label:<15} {eb_n0:5.1f}  {result['ber']:10.6f} "
                  f"{result['errors']:8d} {result['bits']:8d} {result['elapsed']:7.1f}s")
            sys.stdout.flush()
